...
// "wiki" is generated by scripts/build-wiki.mjs (the first stage of build:site) from the
// markdown in wiki/, and committed — see that file's header for why the output is in the repo.
// The five old guide paths are now one-line redirect stubs to their /wiki/ homes: site/_redirects
// serves 301s on the website, but that file is Cloudflare-only and the desktop app serves
// public/ at ITS root, so the stubs are what keeps those URLs working offline and in older
// installs whose in-app links still point at /guides/.
const ROOT_PAGES = [
  "wiki", "wiki.css",
